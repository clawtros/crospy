...
(function(window) {
  ...
  function finalizeGrid(data) {
    ...
    ReactDOM.render(<Crossword playerId={socket.id} crosswordId={data._id} model={model} rawData={data} title={data.gridinfo.name} clues={data.clues} numbered={data.numbered} cells={data.cells} size={data.gridinfo.size}/>, document.getElementById('app'));
